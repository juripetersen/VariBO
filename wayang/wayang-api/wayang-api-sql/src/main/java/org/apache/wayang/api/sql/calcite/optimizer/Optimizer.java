...
public class Optimizer {
    ...
    /**
     * Rule {@link JoinCommuteRule} takes too long when joins number grows. We
     * disable this rule if query has joins
    ...
    public static Optimizer create(
        ...
        final SqlOperatorTable operatorTable = SqlOperatorTables
        ...
        converterConfig.getRelBuilderFactory().create(cluster, catalogReader);
    ...
    public RelNode optimize(final RelNode node, final RelTraitSet requiredTraitSet, final RuleSet rules) {
        ...
        final Program program = Programs.heuristicJoinOrder(rules, false, 0);

        final RelNode optimalJoinOrder = program.run(
                volcanoPlanner,
                decorrelate,
                requiredTraitSet,
     ...
     * @return
     */
    public RelNode prepare(final RelNode node, final Collection<RelOptRule> rules) {
