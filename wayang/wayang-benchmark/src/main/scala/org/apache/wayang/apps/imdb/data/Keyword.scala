...
/**
  * Represents elements from the IMDB `keyword` table.
  */
case class Keyword(id: Integer,
                    keyword: String,
                    phoneticCode: String) extends Serializable

object Keyword extends Serializable {

  val fields = IndexedSeq("id", "keyword", "phonetic_code")

  /**
    * Parse a CSV row into a [[Keyword]] instance.
    *
    * @param csv the [[String]] to parse
    * @return the [[Keyword]]
    */
  def parseCsv(csv: String): Keyword = {
  ...
  def toTuple(a: Keyword) : (Integer, String, String) = {
      (a.id, a.keyword, a.phoneticCode)
  }

  def toArray(a: Keyword) : Array[AnyRef] = {
      Array(a.id, a.keyword, a.phoneticCode)
  }
