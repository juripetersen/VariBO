...
/**
  * Represents elements from the IMDB `role_type` table.
  */
case class RoleType(id: Integer,
                role: String) extends Serializable

object RoleType extends Serializable {

  val fields = IndexedSeq("id", "role")

  /**
    * Parse a CSV row into a [[RoleType]] instance.
    *
    * @param csv the [[String]] to parse
    * @return the [[RoleType]]
    */
  def parseCsv(csv: String): RoleType = {
  ...
  def toTuple(a: RoleType) : (Integer, String) = {
      (a.id, a.role)
  }

  def toArray(a: RoleType) : Array[AnyRef] = {
      Array(a.id, a.role)
  }
