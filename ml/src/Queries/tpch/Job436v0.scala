 ...
 */
 ...
 class Job436v0 extends GeneratableJob {
...
val map8 = map7.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map12 = source13.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val map10 = group11.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val map16 = source17.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val reduce15 = map16.reduce((x1, x2) => x1)
      

val join14 = join9.keyBy[Long](_._4).join(reduce15.keyBy[Long](_._1)).map(x => x.field1)
