 ...
 */
 ...
 class Job795v0 extends GeneratableJob {
...
val map5 = map6.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val filter4 = map5.filter(x=>x._3 <= 5003.0)
      

val join3 = map2.keyBy[Long](_._2).join(filter4.keyBy[Long](_._1)).map(x => x.field1)
       

val map8 = join3.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val map12 = source13.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val map10 = group11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join9 = map8.keyBy[Long](_._2).join(map10.keyBy[Long](_._1)).map(x => x.field1)
...
val map16 = group17.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map15 = map16.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
