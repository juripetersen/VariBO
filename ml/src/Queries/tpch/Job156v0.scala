 ...
 */
 ...
 class Job156v0 extends GeneratableJob {
...
val filter3 = source4.filter(x=>x._4 <= 500.41)
...
val map6 = map5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val filter10 = source11.filter(x=>x._6 <= 7270.635)
