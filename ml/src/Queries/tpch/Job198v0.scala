 ...
 */
 ...
 class Job198v0 extends GeneratableJob {
...
val map3 = reduce2.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val group4 = map3.reduceByKey(_._1, (t1,t2) => t1)
...
val reduce7 = source9.reduce((x1, x2) => x1)
