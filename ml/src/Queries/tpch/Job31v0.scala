 ...
 */
 ...
 class Job31v0 extends GeneratableJob {
...
val filter7 = map8.filter(x=>x._6 <= 7270.635)
      

val join6 = join1.keyBy[Long](_._2).join(filter7.keyBy[Long](_._1)).map(x => x.field1)
       

val group10 = join6.reduceByKey(_._3, (t1,t2) => t1)
          

val map11 = group10.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val group15 = source17.reduceByKey(_._2, (t1,t2) => t1)
          

val map14 = group15.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val join13 = group12.keyBy[Long](_._4).join(map14.keyBy[Long](_._1)).map(x => x.field1)
