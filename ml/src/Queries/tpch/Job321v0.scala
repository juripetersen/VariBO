 ...
 */
 ...
 class Job321v0 extends GeneratableJob {
...
val group2 = map1.reduceByKey(_._2, (t1,t2) => t1)
...
val map6 = map7.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map5 = map6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val group9 = join4.reduceByKey(_._1, (t1,t2) => t1)
