 ...
 */
 ...
 class Job161v0 extends GeneratableJob {
...
val join2 = map1.keyBy[Long](_._2).join(source3.keyBy[Long](_._1)).map(x => x.field1)
...
val join4 = join2.keyBy[Long](_._2).join(map5.keyBy[Long](_._1)).map(x => x.field1)
       

val map9 = join4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val join10 = map9.keyBy[Long](_._4).join(map11.keyBy[Long](_._1)).map(x => x.field1)
       

val map13 = join10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
