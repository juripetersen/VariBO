 ...
 */
 ...
 class Job51v0 extends GeneratableJob {
...
val filter2 = map1.filter(x=>x._11 <= CsvUtils.parseDate("1995-06-19"))
...
val group5 = sort6.reduceByKey(_._1, (t1,t2) => t1)
          

val map4 = group5.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join3 = filter2.keyBy[Long](_._2).join(map4.keyBy[Long](_._1)).map(x => x.field1)
       

val group8 = join3.reduceByKey(_._1, (t1,t2) => t1)
...
val join9 = group8.keyBy[Long](_._2).join(map10.keyBy[Long](_._1)).map(x => x.field1)
...
val map13 = filter14.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join12 = join9.keyBy[Long](_._4).join(map13.keyBy[Long](_._1)).map(x => x.field1)
