 ...
 */
 ...
 class Job619v0 extends GeneratableJob {
...
val join4 = join1.keyBy[Long](_._2).join(map5.keyBy[Long](_._1)).map(x => x.field1)
       

val group9 = join4.reduceByKey(_._3, (t1,t2) => t1)
          

val map10 = group9.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val reduce11 = map10.reduce((x1, x2) => x1)
      

val sort12 = reduce11.sort(_._5)
      

 return sort12.collectNoExecute()
