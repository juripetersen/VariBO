 ...
 */
 ...
 class Job126v0 extends GeneratableJob {
...
val map5 = group4.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map9 = join6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
