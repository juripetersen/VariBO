 ...
 */
 ...
 class Job568v0 extends GeneratableJob {
...
val group8 = source10.reduceByKey(_._3, (t1,t2) => t1)
          

val map7 = group8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val reduce14 = source15.reduce((x1, x2) => x1)
      

val sort13 = reduce14.sort(_._3)
      

val filter12 = sort13.filter(x=>x._3 == 1.0)
      

val join11 = join6.keyBy[Long](_._4).join(filter12.keyBy[Long](_._1)).map(x => x.field1)
       

val group16 = join11.reduceByKey(_._2, (t1,t2) => t1)
          

val map17 = group16.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

 return map17.collectNoExecute()
