 ...
 */
 ...
 class Job523v0 extends GeneratableJob {
...
val map6 = source7.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val reduce5 = map6.reduce((x1, x2) => x1)
      

val map4 = reduce5.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val join8 = join3.keyBy[Long](_._2).join(group9.keyBy[Long](_._1)).map(x => x.field1)
       

val map12 = join8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val partition13 = map12.mapPartitions(x=>x)
      

 return partition13.collectNoExecute()
