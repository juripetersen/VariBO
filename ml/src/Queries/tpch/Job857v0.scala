 ...
 */
 ...
 class Job857v0 extends GeneratableJob {
...
val map4 = map3.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val group6 = map8.reduceByKey(_._3, (t1,t2) => t1)
          

val join5 = map4.keyBy[Long](_._2).join(group6.keyBy[Long](_._1)).map(x => x.field1)
...
val filter11 = source12.filter(x=>x._3 == 1.0)
