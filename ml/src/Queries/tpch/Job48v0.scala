 ...
 */
 ...
 class Job48v0 extends GeneratableJob {
...
val map5 = group6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val join3 = map2.keyBy[Long](_._2).join(group4.keyBy[Long](_._1)).map(x => x.field1)
