 ...
 */
 ...
 class Job988v0 extends GeneratableJob {
...
val filter4 = map3.filter(x=>x._5 <= 13.0)
...
val join5 = filter4.keyBy[Long](_._2).join(source6.keyBy[Long](_._1)).map(x => x.field1)
       

val map7 = join5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
