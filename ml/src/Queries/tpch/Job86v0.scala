 ...
 */
 ...
 class Job86v0 extends GeneratableJob {
...
val filter2 = source3.filter(x=>x._3 <= 5003.0)
      

val join1 = source0.keyBy[Long](_._2).join(filter2.keyBy[Long](_._1)).map(x => x.field1)
       

val map4 = join1.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val group8 = source9.reduceByKey(_._3, (t1,t2) => t1)
          

val map7 = group8.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map6 = map7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map10 = join5.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val join12 = map11.keyBy[Long](_._4).join(map13.keyBy[Long](_._1)).map(x => x.field1)
