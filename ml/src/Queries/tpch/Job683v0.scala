 ...
 */
 ...
 class Job683v0 extends GeneratableJob {
...
val map3 = join1.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val filter6 = map7.filter(x=>x._6 <= 7270.635)
...
val filter12 = source13.filter(x=>x._3 == 1.0)
      

val filter11 = filter12.filter(x=>x._3 == 2.0)
      

val join10 = join5.keyBy[Long](_._4).join(filter11.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce14 = join10.reduce((x1, x2) => x1)
