 ...
 */
 ...
 class Job304v0 extends GeneratableJob {
...
val reduce4 = map2.reduce((x1, x2) => x1)
      

val map5 = reduce4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
