 ...
 */
 ...
 class Job341v0 extends GeneratableJob {
...
val group8 = map7.reduceByKey(_._1, (t1,t2) => t1)
          

val filter9 = group8.filter(x=>x._3 <= 7499.0)
...
val map11 = source12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val join10 = filter9.keyBy[Long](_._2).join(map11.keyBy[Long](_._1)).map(x => x.field1)
