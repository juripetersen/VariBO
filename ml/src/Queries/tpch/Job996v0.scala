 ...
 */
 ...
 class Job996v0 extends GeneratableJob {
...
val reduce11 = join6.reduce((x1, x2) => x1)
      

val map12 = reduce11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map13 = map12.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val group14 = map13.reduceByKey(_._3, (t1,t2) => t1)
          

 return group14.collectNoExecute()
