 ...
 */
 ...
 class Job707v0 extends GeneratableJob {
...
val group10 = map9.reduceByKey(_._3, (t1,t2) => t1)
          

val map11 = group10.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map12 = map11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
