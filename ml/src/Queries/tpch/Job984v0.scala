 ...
 */
 ...
 class Job984v0 extends GeneratableJob {
...
val map4 = join1.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val map11 = map10.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
