 ...
 */
 ...
 class Job243v0 extends GeneratableJob {
...
val group4 = source5.reduceByKey(_._1, (t1,t2) => t1)
          

val map3 = group4.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map2 = map3.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join1 = source0.keyBy[Long](_._2).join(map2.keyBy[Long](_._1)).map(x => x.field1)
       

val map6 = join1.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map7 = map6.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map8 = map7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val reduce12 = source13.reduce((x1, x2) => x1)
      

val map11 = reduce12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val filter10 = map11.filter(x=>x._6 <= 7270.635)
      

val join9 = map8.keyBy[Long](_._2).join(filter10.keyBy[Long](_._1)).map(x => x.field1)
...
val group17 = source18.reduceByKey(_._2, (t1,t2) => t1)
          

val map16 = group17.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map15 = map16.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join14 = join9.keyBy[Long](_._4).join(map15.keyBy[Long](_._1)).map(x => x.field1)
