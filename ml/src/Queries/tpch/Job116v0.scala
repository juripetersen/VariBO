 ...
 */
 ...
 class Job116v0 extends GeneratableJob {
...
val filter5 = join2.filter(x=>x._4 <= 500.41)
      

val map6 = filter5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val join8 = filter7.keyBy[Long](_._2).join(reduce9.keyBy[Long](_._1)).map(x => x.field1)
