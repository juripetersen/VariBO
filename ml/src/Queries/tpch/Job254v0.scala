 ...
 */
 ...
 class Job254v0 extends GeneratableJob {
...
val group3 = map2.reduceByKey(_._14, (t1,t2) => t1)
          

val map4 = group3.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val group12 = source13.reduceByKey(_._3, (t1,t2) => t1)
          

val map11 = group12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val join9 = join5.keyBy[Long](_._2).join(map10.keyBy[Long](_._1)).map(x => x.field1)
