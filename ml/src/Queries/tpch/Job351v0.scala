 ...
 */
 ...
 class Job351v0 extends GeneratableJob {
...
val reduce8 = source9.reduce((x1, x2) => x1)
      

val sort7 = reduce8.sort(_._5)
      

val filter6 = sort7.filter(x=>x._4 == 6.0)
      

val join5 = map4.keyBy[Long](_._2).join(filter6.keyBy[Long](_._1)).map(x => x.field1)
       

val map10 = join5.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
