 ...
 */
 ...
 class Job393v0 extends GeneratableJob {
...
val filter2 = map3.filter(x=>x._3 <= 5003.0)
...
val join8 = reduce7.keyBy[Long](_._2).join(map9.keyBy[Long](_._1)).map(x => x.field1)
...
val filter13 = source14.filter(x=>x._3 == 3.0)
      

val join12 = join8.keyBy[Long](_._4).join(filter13.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce15 = join12.reduce((x1, x2) => x1)
      

 return reduce15.collectNoExecute()
