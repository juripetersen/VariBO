 ...
 */
 ...
 class Job985v0 extends GeneratableJob {
...
val map5 = join1.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map6 = map5.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val reduce7 = map6.reduce((x1, x2) => x1)
...
val sort10 = partition11.sort(_._3)
      

val filter9 = sort10.filter(x=>x._6 <= 7270.635)
      

val join8 = reduce7.keyBy[Long](_._2).join(filter9.keyBy[Long](_._1)).map(x => x.field1)
