 ...
 */
 ...
 class Job74v0 extends GeneratableJob {
...
val map9 = source10.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map8 = map9.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map7 = map8.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join6 = join1.keyBy[Long](_._2).join(map7.keyBy[Long](_._1)).map(x => x.field1)
       

val map11 = join6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val group12 = map11.reduceByKey(_._3, (t1,t2) => t1)
          

val map13 = group12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map14 = map13.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

 return map14.collectNoExecute()
