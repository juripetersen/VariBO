 ...
 */
 ...
 class Job971v0 extends GeneratableJob {
...
val filter4 = source5.filter(x=>x._4 <= 500.41)
      

val join3 = map2.keyBy[Long](_._2).join(filter4.keyBy[Long](_._1)).map(x => x.field1)
       

val map6 = join3.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
