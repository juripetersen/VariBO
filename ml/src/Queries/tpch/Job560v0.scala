 ...
 */
 ...
 class Job560v0 extends GeneratableJob {
...
val join5 = join2.keyBy[Long](_._2).join(source6.keyBy[Long](_._1)).map(x => x.field1)
       

val map7 = join5.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
