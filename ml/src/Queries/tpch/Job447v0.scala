 ...
 */
 ...
 class Job447v0 extends GeneratableJob {
...
val group7 = map6.reduceByKey(_._1, (t1,t2) => t1)
          

val map8 = group7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map11 = map12.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map10 = map11.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
