 ...
 */
 ...
 class Job814v0 extends GeneratableJob {
...
val reduce9 = source11.reduce((x1, x2) => x1)
      

val reduce8 = reduce9.reduce((x1, x2) => x1)
      

val join7 = map6.keyBy[Long](_._2).join(reduce8.keyBy[Long](_._1)).map(x => x.field1)
...
val map13 = source14.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join12 = join7.keyBy[Long](_._4).join(map13.keyBy[Long](_._1)).map(x => x.field1)
       

val filter15 = join12.filter(x=>x._3 == 1.0)
      

val filter16 = filter15.filter(x=>x._3 == 2.0)
      

 return filter16.collectNoExecute()
