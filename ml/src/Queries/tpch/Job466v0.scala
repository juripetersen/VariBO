 ...
 */
 ...
 class Job466v0 extends GeneratableJob {
...
val join5 = reduce4.keyBy[Long](_._2).join(map6.keyBy[Long](_._1)).map(x => x.field1)
       

val map9 = join5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
