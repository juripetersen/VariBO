 ...
 */
 ...
 class Job768v0 extends GeneratableJob {
...
val group3 = map2.reduceByKey(_._2, (t1,t2) => t1)
...
val filter5 = map4.filter(x=>x._5 <= 13.0)
...
val filter9 = source10.filter(x=>x._4 <= 500.41)
      

val map8 = filter9.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val filter7 = map8.filter(x=>x._3 <= 7499.0)
      

val join6 = filter5.keyBy[Long](_._2).join(filter7.keyBy[Long](_._1)).map(x => x.field1)
