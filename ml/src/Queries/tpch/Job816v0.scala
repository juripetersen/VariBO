 ...
 */
 ...
 class Job816v0 extends GeneratableJob {
...
val map8 = reduce7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val filter9 = map8.filter(x=>x._4 <= 250.81)
...
val join10 = filter9.keyBy[Long](_._2).join(filter11.keyBy[Long](_._1)).map(x => x.field1)
