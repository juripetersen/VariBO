 ...
 */
 ...
 class Job611v0 extends GeneratableJob {
...
val map5 = map4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val filter6 = map5.filter(x=>x._4 <= 500.41)
...
val join7 = filter6.keyBy[Long](_._2).join(group8.keyBy[Long](_._1)).map(x => x.field1)
       

 return join7.collectNoExecute()
