 ...
 */
 ...
 class Job679v0 extends GeneratableJob {
...
val join6 = join2.keyBy[Long](_._2).join(map7.keyBy[Long](_._1)).map(x => x.field1)
...
val map13 = map14.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map12 = map13.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join11 = join6.keyBy[Long](_._4).join(map12.keyBy[Long](_._1)).map(x => x.field1)
       

val map16 = join11.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val filter17 = map16.filter(x=>x._3 == 1.0)
      

 return filter17.collectNoExecute()
