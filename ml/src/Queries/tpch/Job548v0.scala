 ...
 */
 ...
 class Job548v0 extends GeneratableJob {
...
val map5 = source6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val group4 = map5.reduceByKey(_._1, (t1,t2) => t1)
          

val map3 = group4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map10 = source11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val reduce8 = map10.reduce((x1, x2) => x1)
      

val join7 = join2.keyBy[Long](_._2).join(reduce8.keyBy[Long](_._1)).map(x => x.field1)
...
val map15 = source16.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val group14 = map15.reduceByKey(_._2, (t1,t2) => t1)
          

val map13 = group14.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val join12 = join7.keyBy[Long](_._4).join(map13.keyBy[Long](_._1)).map(x => x.field1)
       

val map17 = join12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map18 = map17.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
