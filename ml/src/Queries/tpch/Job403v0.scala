 ...
 */
 ...
 class Job403v0 extends GeneratableJob {
...
val map11 = join6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val filter12 = map11.filter(x=>x._6 <= 7270.635)
...
val filter14 = source15.filter(x=>x._3 == 1.0)
      

val join13 = filter12.keyBy[Long](_._4).join(filter14.keyBy[Long](_._1)).map(x => x.field1)
       

val map16 = join13.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
