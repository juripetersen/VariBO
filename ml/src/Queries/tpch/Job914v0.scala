 ...
 */
 ...
 class Job914v0 extends GeneratableJob {
...
val map2 = source3.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map7 = source8.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val reduce5 = map7.reduce((x1, x2) => x1)
      

val join4 = join1.keyBy[Long](_._2).join(reduce5.keyBy[Long](_._1)).map(x => x.field1)
       

val filter9 = join4.filter(x=>x._4 == 6.0)
...
val group13 = source14.reduceByKey(_._2, (t1,t2) => t1)
          

val map12 = group13.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map11 = map12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join10 = filter9.keyBy[Long](_._4).join(map11.keyBy[Long](_._1)).map(x => x.field1)
       

val map15 = join10.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map16 = map15.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
