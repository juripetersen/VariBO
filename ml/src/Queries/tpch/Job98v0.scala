 ...
 */
 ...
 class Job98v0 extends GeneratableJob {
...
val map4 = source5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map6 = join3.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val partition15 = source16.mapPartitions(x=>x)
      

val sort14 = partition15.sort(_._2)
      

val map13 = sort14.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
