 ...
 */
 ...
 class Job803v0 extends GeneratableJob {
...
val reduce2 = map4.reduce((x1, x2) => x1)
...
val map10 = source11.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map9 = map10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map8 = map9.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val map14 = group13.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
