 ...
 */
 ...
 class Job686v0 extends GeneratableJob {
...
val group3 = join1.reduceByKey(_._1, (t1,t2) => t1)
          

val map4 = group3.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map5 = map4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val reduce7 = map9.reduce((x1, x2) => x1)
      

val join6 = map5.keyBy[Long](_._2).join(reduce7.keyBy[Long](_._1)).map(x => x.field1)
       

val filter11 = join6.filter(x=>x._6 <= 4541.0650000000005)
      

 return filter11.collectNoExecute()
