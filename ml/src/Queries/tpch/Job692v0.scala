 ...
 */
 ...
 class Job692v0 extends GeneratableJob {
...
val filter9 = map10.filter(x=>x._4 == 6.0)
      

val map8 = filter9.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val reduce13 = map14.reduce((x1, x2) => x1)
      

val join12 = join7.keyBy[Long](_._4).join(reduce13.keyBy[Long](_._1)).map(x => x.field1)
       

val map16 = join12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val partition17 = map16.mapPartitions(x=>x)
      

 return partition17.collectNoExecute()
