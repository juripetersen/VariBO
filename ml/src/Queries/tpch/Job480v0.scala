 ...
 */
 ...
 class Job480v0 extends GeneratableJob {
...
val reduce7 = source8.reduce((x1, x2) => x1)
      

val map6 = reduce7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val filter10 = map9.filter(x=>x._4 <= 500.41)
      

 return filter10.collectNoExecute()
