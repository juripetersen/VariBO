 ...
 */
 ...
 class Job46v0 extends GeneratableJob {
...
val filter8 = join3.filter(x=>x._3 <= 7499.0)
...
val map12 = join9.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
