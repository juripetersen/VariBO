 ...
 */
 ...
 class Job594v0 extends GeneratableJob {
...
val map5 = join1.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val filter7 = source8.filter(x=>x._4 == 6.0)
      

val join6 = map5.keyBy[Long](_._2).join(filter7.keyBy[Long](_._1)).map(x => x.field1)
       

val group9 = join6.reduceByKey(_._3, (t1,t2) => t1)
          

val map10 = group9.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
