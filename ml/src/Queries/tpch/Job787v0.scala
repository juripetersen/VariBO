 ...
 */
 ...
 class Job787v0 extends GeneratableJob {
...
val reduce6 = source8.reduce((x1, x2) => x1)
      

val map5 = reduce6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
