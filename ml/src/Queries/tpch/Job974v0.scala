 ...
 */
 ...
 class Job974v0 extends GeneratableJob {
...
val reduce4 = source5.reduce((x1, x2) => x1)
      

val map3 = reduce4.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val group2 = map3.reduceByKey(_._1, (t1,t2) => t1)
          

val join1 = source0.keyBy[Long](_._2).join(group2.keyBy[Long](_._1)).map(x => x.field1)
       

val map6 = join1.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val map11 = join7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map12 = map11.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val map14 = source15.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join13 = map12.keyBy[Long](_._4).join(map14.keyBy[Long](_._1)).map(x => x.field1)
