 ...
 */
 ...
 class Job307v0 extends GeneratableJob {
...
val filter9 = source10.filter(x=>x._4 == 12.0)
      

val map8 = filter9.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val reduce12 = join6.reduce((x1, x2) => x1)
      

val map13 = reduce12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
