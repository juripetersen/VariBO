 ...
 */
 ...
 class Job684v0 extends GeneratableJob {
...
val join9 = map8.keyBy[Long](_._2).join(source10.keyBy[Long](_._1)).map(x => x.field1)
       

val map11 = join9.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
