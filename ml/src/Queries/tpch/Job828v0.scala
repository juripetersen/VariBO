 ...
 */
 ...
 class Job828v0 extends GeneratableJob {
...
val map4 = map5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val filter3 = map4.filter(x=>x._4 <= 500.41)
...
val group11 = source13.reduceByKey(_._3, (t1,t2) => t1)
          

val group10 = group11.reduceByKey(_._3, (t1,t2) => t1)
          

val join9 = map8.keyBy[Long](_._2).join(group10.keyBy[Long](_._1)).map(x => x.field1)
