 ...
 */
 ...
 class Job469v0 extends GeneratableJob {
...
val join4 = map3.keyBy[Long](_._2).join(source5.keyBy[Long](_._1)).map(x => x.field1)
...
val map7 = filter8.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val join6 = join4.keyBy[Long](_._2).join(map7.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce10 = join6.reduce((x1, x2) => x1)
