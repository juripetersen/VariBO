 ...
 */
 ...
 class Job641v0 extends GeneratableJob {
...
val filter8 = map9.filter(x=>x._4 == 12.0)
      

val join7 = map6.keyBy[Long](_._2).join(filter8.keyBy[Long](_._1)).map(x => x.field1)
       

val group11 = join7.reduceByKey(_._3, (t1,t2) => t1)
          

val map12 = group11.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val reduce13 = map12.reduce((x1, x2) => x1)
      

 return reduce13.collectNoExecute()
