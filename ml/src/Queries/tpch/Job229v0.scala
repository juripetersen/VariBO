 ...
 */
 ...
 class Job229v0 extends GeneratableJob {
...
val map5 = group6.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val reduce11 = source13.reduce((x1, x2) => x1)
      

val reduce10 = reduce11.reduce((x1, x2) => x1)
      

val join9 = join4.keyBy[Long](_._2).join(reduce10.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce14 = join9.reduce((x1, x2) => x1)
      

 return reduce14.collectNoExecute()
