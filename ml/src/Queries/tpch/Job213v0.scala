 ...
 */
 ...
 class Job213v0 extends GeneratableJob {
...
val map7 = map6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val filter9 = source10.filter(x=>x._6 <= 4541.0650000000005)
      

val join8 = map7.keyBy[Long](_._2).join(filter9.keyBy[Long](_._1)).map(x => x.field1)
       

val filter11 = join8.filter(x=>x._4 == 6.0)
...
val join12 = filter11.keyBy[Long](_._4).join(source13.keyBy[Long](_._1)).map(x => x.field1)
