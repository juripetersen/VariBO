 ...
 */
 ...
 class Job380v0 extends GeneratableJob {
...
val reduce7 = join1.reduce((x1, x2) => x1)
      

val map8 = reduce7.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val group12 = map13.reduceByKey(_._3, (t1,t2) => t1)
          

val map11 = group12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join10 = map9.keyBy[Long](_._2).join(map11.keyBy[Long](_._1)).map(x => x.field1)
