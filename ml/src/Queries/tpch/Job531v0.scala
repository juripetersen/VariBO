 ...
 */
 ...
 class Job531v0 extends GeneratableJob {
...
val reduce3 = reduce4.reduce((x1, x2) => x1)
      

val join2 = map1.keyBy[Long](_._2).join(reduce3.keyBy[Long](_._1)).map(x => x.field1)
       

val group7 = join2.reduceByKey(_._1, (t1,t2) => t1)
          

val map8 = group7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val map10 = map11.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val map14 = join9.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
