 ...
 */
 ...
 class Job887v0 extends GeneratableJob {
...
val filter2 = filter1.filter(x=>x._15 ==  "AIR" )
      

val map3 = filter2.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val group6 = map7.reduceByKey(_._1, (t1,t2) => t1)
          

val map5 = group6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val join9 = join4.keyBy[Long](_._2).join(group10.keyBy[Long](_._1)).map(x => x.field1)
       

val filter14 = join9.filter(x=>x._6 <= 7270.635)
      

 return filter14.collectNoExecute()
