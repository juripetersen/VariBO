 ...
 */
 ...
 class Job824v0 extends GeneratableJob {
...
val map5 = join2.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val join6 = map5.keyBy[Long](_._2).join(source7.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce8 = join6.reduce((x1, x2) => x1)
...
val map11 = filter12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val join9 = reduce8.keyBy[Long](_._4).join(map10.keyBy[Long](_._1)).map(x => x.field1)
