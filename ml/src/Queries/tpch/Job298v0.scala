 ...
 */
 ...
 class Job298v0 extends GeneratableJob {
...
val filter7 = map8.filter(x=>x._4 == 12.0)
      

val join6 = join4.keyBy[Long](_._2).join(filter7.keyBy[Long](_._1)).map(x => x.field1)
       

val map10 = join6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
