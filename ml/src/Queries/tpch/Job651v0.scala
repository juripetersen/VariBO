 ...
 */
 ...
 class Job651v0 extends GeneratableJob {
...
val map8 = source9.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join7 = map6.keyBy[Long](_._2).join(map8.keyBy[Long](_._1)).map(x => x.field1)
       

val map10 = join7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val map14 = source15.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val group12 = map14.reduceByKey(_._2, (t1,t2) => t1)
          

val join11 = map10.keyBy[Long](_._4).join(group12.keyBy[Long](_._1)).map(x => x.field1)
       

val map16 = join11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

 return map16.collectNoExecute()
