 ...
 */
 ...
 class Job452v0 extends GeneratableJob {
...
val map6 = join2.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val filter7 = map6.filter(x=>x._3 <= 5003.0)
...
val filter9 = source10.filter(x=>x._4 == 6.0)
      

val join8 = filter7.keyBy[Long](_._2).join(filter9.keyBy[Long](_._1)).map(x => x.field1)
...
val join11 = join8.keyBy[Long](_._4).join(map12.keyBy[Long](_._1)).map(x => x.field1)
