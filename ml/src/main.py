...
from OurModels.CostModel.model import CostModel, CostLoss
...
def main(args) -> None:
    ...
    if args.model == "classifier":
        data, in_dim, out_dim = load_autoencoder_data(path=get_relative_path('classifier.retrain.txt', 'Data/splits/imdb/complex/amended'), retrain_path=args.retrain, device=device, num_ops=args.operators, num_platfs=args.platforms)
        test_data, _, _ = load_autoencoder_data(path=get_relative_path('classifier.retrain.txt', 'Data/splits/imdb/complex/amended'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
        val_data, _, _ = load_autoencoder_data(path=get_relative_path('classifier.retrain.txt', 'Data/splits/imdb/complex/amended'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
    ...
    if args.model == 'bcvae':
        data, in_dim, out_dim = load_autoencoder_data(path=get_relative_path('train.txt', 'Data/splits/imdb/training/bcvae'), retrain_path=args.retrain, device=device, num_ops=args.operators, num_platfs=args.platforms)
        test_data, _, _ = load_autoencoder_data(path=get_relative_path('test.txt', 'Data/splits/imdb/training/bcvae'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
        val_data, _, _ = load_autoencoder_data(path=get_relative_path('validate.txt', 'Data/splits/imdb/training/bcvae'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)

        model_class = BetaCVAE
        loss_function = BetaCVAELoss

    if args.model == 'carbvae':
        data, in_dim, out_dim, mean, std = load_autoencoder_carb_data(path=get_relative_path('train.txt', 'Data/splits/imdb/complex/amended'), retrain_path=args.retrain, device=device, num_ops=args.operators, num_platfs=args.platforms)
        test_data, _, _, _, _ = load_autoencoder_carb_data(path=get_relative_path('test.txt', 'Data/splits/imdb/complex/amended'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
        val_data, _, _, _, _ = load_autoencoder_carb_data(path=get_relative_path('validate.txt', 'Data/splits/imdb/complex/amended'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
    ...
    if args.model == "cost":
        data, in_dim, out_dim = load_costmodel_data(path=get_relative_path('train.txt', 'Data/splits/stats/amended'), device=device)
        test_data, _, _ = load_costmodel_data(path=get_relative_path('validate.txt', 'Data/splits/stats/amended'), device=device)
        val_data, _, _ = load_costmodel_data(path=get_relative_path('validate.txt', 'Data/splits/stats/amended'), device=device)
        model_class = CostModel
        loss_function = CostLoss
