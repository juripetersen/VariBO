...
def load_autoencoder_data(device: str, path: str, retrain_path: str = "", num_ops: int = 43, num_platfs: int = 5) -> tuple[TreeVectorDataset, int, int]:
    regex_pattern = r'\(((?:[+,-]?\d+(?:,[+,-]?\d+)*)(?:\s*,\s*\(.*?\))*)\)'

    print(f"[LOADING] num_ops: {num_ops}, num_platfs: {num_platfs}")
    def platform_encodings(optimal_tree: str):
        ...
        for match in matches_iterator:
            ...
            values = [int(num.strip()) for num in find.split(',')]
            replacement = ','.join(map(str, values[1 + num_ops:1+num_ops+num_platfs]))
...
def load_costmodel_data(device: str, path: str) -> tuple[TreeVectorDataset, int, None]:
    ...
    costs = torch.log1p(torch.tensor(costs, dtype=torch.float32)).to(device)
...
def load_autoencoder_carb_data(device: str, path: str, retrain_path: str = "", num_ops: int = 43, num_platfs: int = 5) -> tuple[TreeVectorDataset, int, int]:
    regex_pattern = r'\(((?:[+,-]?\d+(?:,[+,-]?\d+)*)(?:\s*,\s*\(.*?\))*)\)'
...
class Classifier_Loss(torch.nn.Module):
    def __init__(self):
        super(Classifier_Loss, self).__init__()

    def forward(self, prediction, target):
        pred_logits = prediction[0][:, :, :-1]  # [batch, out_dim, nodes] — drop prepended zero sentinel

        recon_loss = F.cross_entropy(pred_logits, target.argmax(dim=1))
