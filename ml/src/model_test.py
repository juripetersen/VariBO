...
from helper import get_weights_of_model_by_path, set_weights, load_autoencoder_data, get_relative_path, Classifier_Loss
...
def get_platform_choices(file_path: str, line: int):
    ...
    for match in matches_iterator:
        ...
        platform_choices = values[1+43:1+43+5]
        if sum(platform_choices) > 0:
            print("File choices")
            print(platform_choices)


def evaluate_classifier_loss(data_path: str, model_path: str, parameters_path: str):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    data, in_dim, out_dim = load_autoencoder_data(path=data_path, device=device, num_ops=43, num_platfs=5)
    # find model parameters
    weights = get_weights_of_model_by_path(model_path)
    ...
    with open(parameters_path) as file:
        ...
        with torch.no_grad():
            for tree,target in dataloader:
                decoded = model.forward(tree)
                recon_loss = loss_function(decoded, target)['loss']

                #if abs(recon_loss.item()) == 0:

                pred_classes = decoded[0][:, :, :-1].argmax(dim=1)
                ...
                print(recon_loss)
                print(platform_choices)
                print(f"target classes: {target.argmax(dim=1)}")
...
if __name__ == "__main__":
    ...
    file_path = get_relative_path("classifier.poop.txt", "Data/splits/imdb/complex")
    evaluate_classifier_loss(data_path=file_path, model_path=args.model_path, parameters_path=args.parameters)
