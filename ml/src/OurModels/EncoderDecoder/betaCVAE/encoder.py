...
class TreeEncoder(nn.Module):
    def __init__(self, in_dim, hidden_dim, dropout):
        ...
        self.conv2 = BinaryTreeConv(hidden_dim, int(hidden_dim / 2))
        self.norm2 = TreeLayerNorm()

        self.conv3 = BinaryTreeConv(int(hidden_dim / 2), hidden_dim)
        self.norm3 = TreeLayerNorm()
    ...
    def forward(self, node_feats, children):
        ...
        x = self.dropout(F.mish(self.norm1(x)[0]))

        x = self.conv2([x, children])
        x = self.dropout(F.mish(self.norm2(x)[0]))

        x = self.conv3([x, children])
        x = self.dropout(F.mish(self.norm3(x)[0]))
