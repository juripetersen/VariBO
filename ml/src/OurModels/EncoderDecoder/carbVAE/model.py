...
class Loss(torch.nn.Module):
    ...
    def forward(
        ...
        #pred_logits = logits.permute(0, 2, 1)[:, :, 1:]
        pred_logits = logits[:, :, 1:]

        """
        recon = F.cross_entropy(
            pred_logits,
            targets  # no second permute
        )
        """

        target_classes = targets.argmax(dim=1)  # [batch, nodes]
        ...
        masked_target_logits = targets.permute(0, 2, 1)[mask]

        recon = F.cross_entropy(masked_logits, masked_target_logits)
        ...
        #print(f"recon: {recon.item():.4f}, kl: {kl.item():.4f}, attr: {attr_loss.item()}")

        #return recon + self.beta * kl + self.gamma * attr_loss, recon, kl, attr_loss
        return recon + self.beta * kl + self.gamma * attr_loss, recon, kl, attr_loss
