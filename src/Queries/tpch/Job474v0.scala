 ...
 */
 ...
 class Job474v0 extends GeneratableJob {
...
val reduce6 = join1.reduce((x1, x2) => x1)
      

val map7 = reduce6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map14 = join9.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
