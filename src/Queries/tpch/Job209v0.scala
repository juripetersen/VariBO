 ...
 */
 ...
 class Job209v0 extends GeneratableJob {
...
val join8 = map7.keyBy[Long](_._2).join(source9.keyBy[Long](_._1)).map(x => x.field1)
       

val map10 = join8.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val group13 = source15.reduceByKey(_._2, (t1,t2) => t1)
