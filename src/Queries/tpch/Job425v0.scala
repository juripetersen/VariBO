 ...
 */
 ...
 class Job425v0 extends GeneratableJob {
...
val map7 = join2.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map9 = map8.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val filter13 = source14.filter(x=>x._6 <= 7270.635)
      

val map12 = filter13.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val reduce11 = map12.reduce((x1, x2) => x1)
      

val join10 = map9.keyBy[Long](_._2).join(reduce11.keyBy[Long](_._1)).map(x => x.field1)
