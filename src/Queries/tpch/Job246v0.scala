 ...
 */
 ...
 class Job246v0 extends GeneratableJob {
...
val filter4 = sort3.filter(x=>x._5 <= 13.0)
      

val filter5 = filter4.filter(x=>x._11 <= CsvUtils.parseDate("1995-06-19"))
...
val join6 = filter5.keyBy[Long](_._2).join(map7.keyBy[Long](_._1)).map(x => x.field1)
