 ...
 */
 ...
 class Job181v0 extends GeneratableJob {
...
val group2 = map1.reduceByKey(_._14, (t1,t2) => t1)
          

val map3 = group2.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val filter5 = source6.filter(x=>x._3 <= 7499.0)
      

val join4 = map3.keyBy[Long](_._2).join(filter5.keyBy[Long](_._1)).map(x => x.field1)
...
val join7 = join4.keyBy[Long](_._2).join(group8.keyBy[Long](_._1)).map(x => x.field1)
