 ...
 */
 ...
 class Job90v0 extends GeneratableJob {
...
val reduce3 = map5.reduce((x1, x2) => x1)
...
val map9 = filter10.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val join7 = join2.keyBy[Long](_._2).join(group8.keyBy[Long](_._1)).map(x => x.field1)
...
val map13 = map12.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
