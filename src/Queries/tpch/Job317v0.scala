 ...
 */
 ...
 class Job317v0 extends GeneratableJob {
...
val map2 = reduce3.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map7 = group6.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map8 = map7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map9 = map8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map13 = source14.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map12 = map13.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val join10 = map9.keyBy[Long](_._2).join(group11.keyBy[Long](_._1)).map(x => x.field1)
