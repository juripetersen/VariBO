 ...
 */
 ...
 class Job324v0 extends GeneratableJob {
...
val join2 = filter1.keyBy[Long](_._2).join(source3.keyBy[Long](_._1)).map(x => x.field1)
...
val map6 = filter7.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val map9 = join4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
