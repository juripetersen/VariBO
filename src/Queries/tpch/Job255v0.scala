 ...
 */
 ...
 class Job255v0 extends GeneratableJob {
...
val group3 = map5.reduceByKey(_._1, (t1,t2) => t1)
          

val join2 = map1.keyBy[Long](_._2).join(group3.keyBy[Long](_._1)).map(x => x.field1)
...
val filter8 = group9.filter(x=>x._6 <= 7270.635)
      

val join7 = join2.keyBy[Long](_._2).join(filter8.keyBy[Long](_._1)).map(x => x.field1)
       

val group12 = join7.reduceByKey(_._3, (t1,t2) => t1)
          

val map13 = group12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map17 = source18.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val group16 = map17.reduceByKey(_._2, (t1,t2) => t1)
          

val filter15 = group16.filter(x=>x._3 == 2.0)
      

val join14 = map13.keyBy[Long](_._4).join(filter15.keyBy[Long](_._1)).map(x => x.field1)
