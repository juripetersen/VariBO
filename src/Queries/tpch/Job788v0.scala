 ...
 */
 ...
 class Job788v0 extends GeneratableJob {
...
val reduce3 = source5.reduce((x1, x2) => x1)
      

val reduce2 = reduce3.reduce((x1, x2) => x1)
      

val join1 = source0.keyBy[Long](_._2).join(reduce2.keyBy[Long](_._1)).map(x => x.field1)
...
val map10 = join6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map12 = map13.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join11 = map10.keyBy[Long](_._4).join(map12.keyBy[Long](_._1)).map(x => x.field1)
       

val group16 = join11.reduceByKey(_._2, (t1,t2) => t1)
          

 return group16.collectNoExecute()
