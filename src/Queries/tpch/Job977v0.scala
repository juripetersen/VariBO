 ...
 */
 ...
 class Job977v0 extends GeneratableJob {
...
val map4 = join2.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val filter6 = source7.filter(x=>x._4 == 6.0)
      

val join5 = map4.keyBy[Long](_._2).join(filter6.keyBy[Long](_._1)).map(x => x.field1)
       

val filter8 = join5.filter(x=>x._6 <= 7270.635)
...
val reduce10 = map12.reduce((x1, x2) => x1)
      

val join9 = filter8.keyBy[Long](_._4).join(reduce10.keyBy[Long](_._1)).map(x => x.field1)
