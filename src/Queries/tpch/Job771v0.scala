 ...
 */
 ...
 class Job771v0 extends GeneratableJob {
...
val map5 = source6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map7 = join4.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
