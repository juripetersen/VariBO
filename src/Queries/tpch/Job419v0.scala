 ...
 */
 ...
 class Job419v0 extends GeneratableJob {
...
val map7 = join4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val partition11 = source12.mapPartitions(x=>x)
      

val sort10 = partition11.sort(_._1)
      

val filter9 = sort10.filter(x=>x._3 == 1.0)
      

val join8 = map7.keyBy[Long](_._4).join(filter9.keyBy[Long](_._1)).map(x => x.field1)
       

val group14 = join8.reduceByKey(_._2, (t1,t2) => t1)
