 ...
 */
 ...
 class Job706v0 extends GeneratableJob {
...
val map7 = source8.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val filter6 = map7.filter(x=>x._6 <= 4541.0650000000005)
      

val join5 = join2.keyBy[Long](_._2).join(filter6.keyBy[Long](_._1)).map(x => x.field1)
       

val group9 = join5.reduceByKey(_._3, (t1,t2) => t1)
...
val map12 = group13.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val join10 = group9.keyBy[Long](_._4).join(map11.keyBy[Long](_._1)).map(x => x.field1)
       

val map15 = join10.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

 return map15.collectNoExecute()
