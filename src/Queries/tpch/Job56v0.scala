 ...
 */
 ...
 class Job56v0 extends GeneratableJob {
...
val filter8 = source9.filter(x=>x._6 <= 4541.0650000000005)
      

val filter7 = filter8.filter(x=>x._4 == 6.0)
      

val map6 = filter7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
