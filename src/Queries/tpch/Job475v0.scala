 ...
 */
 ...
 class Job475v0 extends GeneratableJob {
...
val reduce5 = map6.reduce((x1, x2) => x1)
      

val join4 = map3.keyBy[Long](_._2).join(reduce5.keyBy[Long](_._1)).map(x => x.field1)
...
val filter10 = source11.filter(x=>x._6 <= 4541.0650000000005)
      

val join9 = join4.keyBy[Long](_._2).join(filter10.keyBy[Long](_._1)).map(x => x.field1)
       

val map12 = join9.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
