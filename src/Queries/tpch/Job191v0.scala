 ...
 */
 ...
 class Job191v0 extends GeneratableJob {
...
val sort8 = source9.sort(_._3)
      

val filter7 = sort8.filter(x=>x._6 <= 7270.635)
      

val map6 = filter7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val join11 = reduce10.keyBy[Long](_._4).join(filter12.keyBy[Long](_._1)).map(x => x.field1)
