 ...
 */
 ...
 class Job592v0 extends GeneratableJob {
...
val map17 = join12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val reduce18 = map17.reduce((x1, x2) => x1)
