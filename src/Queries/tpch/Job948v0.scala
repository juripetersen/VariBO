 ...
 */
 ...
 class Job948v0 extends GeneratableJob {
...
val group7 = join4.reduceByKey(_._3, (t1,t2) => t1)
...
val join8 = group7.keyBy[Long](_._4).join(map9.keyBy[Long](_._1)).map(x => x.field1)
       

val map13 = join8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val reduce14 = map13.reduce((x1, x2) => x1)
