 ...
 */
 ...
 class Job687v0 extends GeneratableJob {
...
val filter3 = filter2.filter(x=>x._11 <= CsvUtils.parseDate("1995-06-19"))
...
val filter6 = source7.filter(x=>x._4 <= 250.81)
      

val map5 = filter6.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join4 = filter3.keyBy[Long](_._2).join(map5.keyBy[Long](_._1)).map(x => x.field1)
       

val map8 = join4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
