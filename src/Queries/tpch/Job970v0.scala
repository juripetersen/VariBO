 ...
 */
 ...
 class Job970v0 extends GeneratableJob {
...
val reduce4 = map3.reduce((x1, x2) => x1)
...
val map8 = source9.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val filter6 = map7.filter(x=>x._3 <= 5003.0)
      

val join5 = reduce4.keyBy[Long](_._2).join(filter6.keyBy[Long](_._1)).map(x => x.field1)
...
val join10 = join5.keyBy[Long](_._2).join(map11.keyBy[Long](_._1)).map(x => x.field1)
