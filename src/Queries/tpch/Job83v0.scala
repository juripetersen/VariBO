 ...
 */
 ...
 class Job83v0 extends GeneratableJob {
...
val map4 = source5.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map3 = map4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map2 = map3.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map6 = join1.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val group9 = source11.reduceByKey(_._3, (t1,t2) => t1)
          

val map8 = group9.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val map12 = join7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val filter13 = map12.filter(x=>x._4 == 6.0)
      

 val source16 = planBuilder
...
val map15 = source16.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join14 = filter13.keyBy[Long](_._4).join(map15.keyBy[Long](_._1)).map(x => x.field1)
