 ...
 */
 ...
 class Job501v0 extends GeneratableJob {
...
val filter4 = join1.filter(x=>x._4 <= 500.41)
      

val map5 = filter4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val map10 = filter11.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
