 ...
 */
 ...
 class Job62v0 extends GeneratableJob {
...
val map8 = filter9.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val filter13 = reduce12.filter(x=>x._6 <= 7270.635)
      

 return filter13.collectNoExecute()
