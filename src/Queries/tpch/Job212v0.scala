 ...
 */
 ...
 class Job212v0 extends GeneratableJob {
...
val filter4 = join1.filter(x=>x._3 <= 5003.0)
...
val join5 = filter4.keyBy[Long](_._2).join(group6.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce10 = join5.reduce((x1, x2) => x1)
      

val map11 = reduce10.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val join12 = map11.keyBy[Long](_._4).join(source13.keyBy[Long](_._1)).map(x => x.field1)
