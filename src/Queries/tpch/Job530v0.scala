 ...
 */
 ...
 class Job530v0 extends GeneratableJob {
...
val reduce9 = map10.reduce((x1, x2) => x1)
      

val join8 = join3.keyBy[Long](_._2).join(reduce9.keyBy[Long](_._1)).map(x => x.field1)
...
val map14 = map13.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
