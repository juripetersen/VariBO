 ...
 */
 ...
 class Job805v0 extends GeneratableJob {
...
val map2 = filter1.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val filter4 = source5.filter(x=>x._4 <= 250.81)
...
val group8 = map9.reduceByKey(_._3, (t1,t2) => t1)
          

val filter7 = group8.filter(x=>x._4 == 6.0)
      

val join6 = join3.keyBy[Long](_._2).join(filter7.keyBy[Long](_._1)).map(x => x.field1)
       

val filter11 = join6.filter(x=>x._6 <= 4541.0650000000005)
...
val map15 = source16.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map14 = map15.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map13 = map14.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join12 = filter11.keyBy[Long](_._4).join(map13.keyBy[Long](_._1)).map(x => x.field1)
       

 return join12.collectNoExecute()
