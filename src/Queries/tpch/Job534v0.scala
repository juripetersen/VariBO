 ...
 */
 ...
 class Job534v0 extends GeneratableJob {
...
val reduce4 = join1.reduce((x1, x2) => x1)
      

val reduce5 = reduce4.reduce((x1, x2) => x1)
...
val map7 = sort8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join6 = reduce5.keyBy[Long](_._2).join(map7.keyBy[Long](_._1)).map(x => x.field1)
