 ...
 */
 ...
 class Job204v0 extends GeneratableJob {
...
val reduce5 = source6.reduce((x1, x2) => x1)
      

val sort4 = reduce5.sort(_._1)
      

val map3 = sort4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val filter11 = source12.filter(x=>x._6 <= 7270.635)
      

val filter10 = filter11.filter(x=>x._4 == 6.0)
      

val map9 = filter10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val filter14 = map15.filter(x=>x._3 == 2.0)
      

val join13 = join8.keyBy[Long](_._4).join(filter14.keyBy[Long](_._1)).map(x => x.field1)
       

val group18 = join13.reduceByKey(_._2, (t1,t2) => t1)
          

 return group18.collectNoExecute()
