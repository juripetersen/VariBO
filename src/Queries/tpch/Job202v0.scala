 ...
 */
 ...
 class Job202v0 extends GeneratableJob {
...
val sort3 = reduce4.sort(_._2)
      

val filter2 = sort3.filter(x=>x._3 <= 5003.0)
...
val reduce8 = reduce7.reduce((x1, x2) => x1)
...
val sort12 = source13.sort(_._1)
      

val filter11 = sort12.filter(x=>x._6 <= 4541.0650000000005)
      

val map10 = filter11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join9 = reduce8.keyBy[Long](_._2).join(map10.keyBy[Long](_._1)).map(x => x.field1)
