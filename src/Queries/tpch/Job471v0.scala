 ...
 */
 ...
 class Job471v0 extends GeneratableJob {
...
val partition5 = source6.mapPartitions(x=>x)
      

val sort4 = partition5.sort(_._1)
      

val group3 = sort4.reduceByKey(_._1, (t1,t2) => t1)
          

val join2 = map1.keyBy[Long](_._2).join(group3.keyBy[Long](_._1)).map(x => x.field1)
       

val filter7 = join2.filter(x=>x._4 <= 500.41)
      

val map8 = filter7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
