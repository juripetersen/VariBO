 ...
 */
 ...
 class Job638v0 extends GeneratableJob {
...
val reduce5 = source7.reduce((x1, x2) => x1)
      

val map4 = reduce5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map10 = filter11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
