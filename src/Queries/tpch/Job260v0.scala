 ...
 */
 ...
 class Job260v0 extends GeneratableJob {
...
val map9 = group10.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val join14 = reduce13.keyBy[Long](_._4).join(source15.keyBy[Long](_._1)).map(x => x.field1)
