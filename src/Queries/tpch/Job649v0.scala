 ...
 */
 ...
 class Job649v0 extends GeneratableJob {
...
val filter5 = map6.filter(x=>x._3 <= 7499.0)
      

val map4 = filter5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map11 = join8.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
