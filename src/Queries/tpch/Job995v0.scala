 ...
 */
 ...
 class Job995v0 extends GeneratableJob {
...
val group5 = map7.reduceByKey(_._1, (t1,t2) => t1)
          

val join4 = reduce3.keyBy[Long](_._2).join(group5.keyBy[Long](_._1)).map(x => x.field1)
...
val map10 = map11.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join9 = join4.keyBy[Long](_._2).join(map10.keyBy[Long](_._1)).map(x => x.field1)
       

val map14 = join9.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
