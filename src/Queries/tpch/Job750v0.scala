 ...
 */
 ...
 class Job750v0 extends GeneratableJob {
...
val map7 = source8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map6 = map7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
...
val join4 = map3.keyBy[Long](_._2).join(map5.keyBy[Long](_._1)).map(x => x.field1)
       

val filter9 = join4.filter(x=>x._4 <= 500.41)
...
val map11 = map12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
