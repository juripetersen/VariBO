 ...
 */
 ...
 class Job429v0 extends GeneratableJob {
...
val reduce8 = source10.reduce((x1, x2) => x1)
      

val map7 = reduce8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join6 = join2.keyBy[Long](_._2).join(map7.keyBy[Long](_._1)).map(x => x.field1)
       

val group11 = join6.reduceByKey(_._3, (t1,t2) => t1)
          

val map12 = group11.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
