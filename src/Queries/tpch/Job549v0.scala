 ...
 */
 ...
 class Job549v0 extends GeneratableJob {
...
val map3 = map4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map9 = map10.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val filter8 = map9.filter(x=>x._4 == 6.0)
...
val group15 = source16.reduceByKey(_._2, (t1,t2) => t1)
...
val map13 = map14.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join12 = join7.keyBy[Long](_._4).join(map13.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce18 = join12.reduce((x1, x2) => x1)
      

 return reduce18.collectNoExecute()
