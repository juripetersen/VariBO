 ...
 */
 ...
 class Job588v0 extends GeneratableJob {
...
val map2 = group3.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val group8 = map10.reduceByKey(_._3, (t1,t2) => t1)
          

val join7 = map6.keyBy[Long](_._2).join(group8.keyBy[Long](_._1)).map(x => x.field1)
       

val map12 = join7.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map13 = map12.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map14 = map13.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
