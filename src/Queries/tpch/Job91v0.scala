 ...
 */
 ...
 class Job91v0 extends GeneratableJob {
...
val group2 = group3.reduceByKey(_._1, (t1,t2) => t1)
...
val map9 = source10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val reduce12 = map11.reduce((x1, x2) => x1)
...
val map16 = source17.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val group14 = map16.reduceByKey(_._2, (t1,t2) => t1)
          

val join13 = reduce12.keyBy[Long](_._4).join(group14.keyBy[Long](_._1)).map(x => x.field1)
       

val reduce18 = join13.reduce((x1, x2) => x1)
