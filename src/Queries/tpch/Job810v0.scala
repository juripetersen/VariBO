 ...
 */
 ...
 class Job810v0 extends GeneratableJob {
...
val join4 = reduce3.keyBy[Long](_._2).join(source5.keyBy[Long](_._1)).map(x => x.field1)
       

val map6 = join4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
