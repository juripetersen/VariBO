 ...
 */
 ...
 class Job819v0 extends GeneratableJob {
...
val group3 = source5.reduceByKey(_._1, (t1,t2) => t1)
          

val map2 = group3.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val group9 = source10.reduceByKey(_._3, (t1,t2) => t1)
...
val map7 = map8.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val filter11 = join6.filter(x=>x._6 <= 7270.635)
      

val sort12 = filter11.sort(_._1)
      

val filter13 = sort12.filter(x=>x._6 <= 4541.0650000000005)
...
val group16 = source18.reduceByKey(_._2, (t1,t2) => t1)
          

val map15 = group16.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
