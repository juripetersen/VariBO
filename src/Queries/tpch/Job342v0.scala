 ...
 */
 ...
 class Job342v0 extends GeneratableJob {
...
val reduce5 = map4.reduce((x1, x2) => x1)
      

val map6 = reduce5.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

 return map6.collectNoExecute()
