 ...
 */
 ...
 class Job269v0 extends GeneratableJob {
...
val filter3 = map4.filter(x=>x._3 <= 5003.0)
      

val filter2 = filter3.filter(x=>x._4 <= 250.81)
      

val join1 = source0.keyBy[Long](_._2).join(filter2.keyBy[Long](_._1)).map(x => x.field1)
       

val group6 = join1.reduceByKey(_._1, (t1,t2) => t1)
...
val join7 = group6.keyBy[Long](_._2).join(map8.keyBy[Long](_._1)).map(x => x.field1)
...
val partition17 = source18.mapPartitions(x=>x)
      

val sort16 = partition17.sort(_._4)
      

val group15 = sort16.reduceByKey(_._2, (t1,t2) => t1)
          

val join14 = map13.keyBy[Long](_._4).join(group15.keyBy[Long](_._1)).map(x => x.field1)
