 ...
 */
 ...
 class Job845v0 extends GeneratableJob {
...
val reduce4 = reduce5.reduce((x1, x2) => x1)
      

val join3 = join1.keyBy[Long](_._2).join(reduce4.keyBy[Long](_._1)).map(x => x.field1)
...
val map14 = join10.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
