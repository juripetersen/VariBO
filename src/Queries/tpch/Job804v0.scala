 ...
 */
 ...
 class Job804v0 extends GeneratableJob {
...
val filter2 = group3.filter(x=>x._4 <= 250.81)
...
val map10 = join7.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
       

val map11 = map10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val sort15 = source16.sort(_._4)
      

val group13 = sort15.reduceByKey(_._2, (t1,t2) => t1)
          

val join12 = map11.keyBy[Long](_._4).join(group13.keyBy[Long](_._1)).map(x => x.field1)
