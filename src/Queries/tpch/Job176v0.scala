 ...
 */
 ...
 class Job176v0 extends GeneratableJob {
...
val group10 = source12.reduceByKey(_._3, (t1,t2) => t1)
...
val map15 = group16.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
...
val join13 = join8.keyBy[Long](_._4).join(reduce14.keyBy[Long](_._1)).map(x => x.field1)
