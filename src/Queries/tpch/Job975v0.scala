 ...
 */
 ...
 class Job975v0 extends GeneratableJob {
...
val join5 = map4.keyBy[Long](_._2).join(source6.keyBy[Long](_._1)).map(x => x.field1)
...
val map8 = group9.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join7 = join5.keyBy[Long](_._2).join(map8.keyBy[Long](_._1)).map(x => x.field1)
