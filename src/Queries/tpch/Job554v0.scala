 ...
 */
 ...
 class Job554v0 extends GeneratableJob {
...
val map5 = filter6.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map8 = join4.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val group9 = map8.reduceByKey(_._1, (t1,t2) => t1)
          

 return group9.collectNoExecute()
