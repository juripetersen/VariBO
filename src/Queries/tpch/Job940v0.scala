 ...
 */
 ...
 class Job940v0 extends GeneratableJob {
...
val filter4 = group5.filter(x=>x._3 <= 5003.0)
      

val join3 = filter2.keyBy[Long](_._2).join(filter4.keyBy[Long](_._1)).map(x => x.field1)
...
val map11 = join8.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
