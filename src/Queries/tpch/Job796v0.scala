 ...
 */
 ...
 class Job796v0 extends GeneratableJob {
...
val filter7 = map6.filter(x=>x._3 <= 7499.0)
...
val join8 = filter7.keyBy[Long](_._2).join(map9.keyBy[Long](_._1)).map(x => x.field1)
       

val map13 = join8.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
