 ...
 */
 ...
 class Job220v0 extends GeneratableJob {
...
val filter4 = source5.filter(x=>x._3 <= 5003.0)
      

val map3 = filter4.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val map6 = join2.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val group10 = source11.reduceByKey(_._3, (t1,t2) => t1)
          

val map9 = group10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
...
val reduce15 = source17.reduce((x1, x2) => x1)
      

val map14 = reduce15.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
