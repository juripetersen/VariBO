 ...
 */
 ...
 class Job873v0 extends GeneratableJob {
...
val filter3 = source4.filter(x=>x._3 <= 5003.0)
      

val map2 = filter3.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val join1 = source0.keyBy[Long](_._2).join(map2.keyBy[Long](_._1)).map(x => x.field1)
       

val group5 = join1.reduceByKey(_._1, (t1,t2) => t1)
          

val map6 = group5.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val filter8 = reduce9.filter(x=>x._4 == 6.0)
      

val join7 = map6.keyBy[Long](_._2).join(filter8.keyBy[Long](_._1)).map(x => x.field1)
...
val map13 = source14.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val group15 = join12.reduceByKey(_._2, (t1,t2) => t1)
          

 return group15.collectNoExecute()
