 ...
 */
 ...
 class Job344v0 extends GeneratableJob {
...
val group7 = map8.reduceByKey(_._3, (t1,t2) => t1)
...
val reduce11 = map13.reduce((x1, x2) => x1)
      

val join10 = join6.keyBy[Long](_._4).join(reduce11.keyBy[Long](_._1)).map(x => x.field1)
       

val map15 = join10.map(x=>x).withUdfComplexity(UDFComplexity.LINEAR)
                

val map16 = map15.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val map17 = map16.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.SUPERQUADRATIC)
