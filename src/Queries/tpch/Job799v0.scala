 ...
 */
 ...
 class Job799v0 extends GeneratableJob {
...
val map5 = join2.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val filter7 = reduce8.filter(x=>x._6 <= 7270.635)
      

val join6 = map5.keyBy[Long](_._2).join(filter7.keyBy[Long](_._1)).map(x => x.field1)
       

val filter11 = join6.filter(x=>x._4 == 6.0)
