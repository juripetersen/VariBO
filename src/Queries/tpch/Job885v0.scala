 ...
 */
 ...
 class Job885v0 extends GeneratableJob {
...
val map8 = join3.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
...
val map14 = map15.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
                  .withUdfComplexity(UDFComplexity.QUADRATIC)
                

val join13 = join9.keyBy[Long](_._4).join(map14.keyBy[Long](_._1)).map(x => x.field1)
