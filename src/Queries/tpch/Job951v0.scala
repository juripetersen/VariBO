 ...
 */
 ...
 class Job951v0 extends GeneratableJob {
...
val filter3 = filter2.filter(x=>x._5 <= 13.0)
      

val map4 = filter3.map(x=> {var count = 0; for (v <- x.productIterator) count+=1; x})
...
val group8 = map9.reduceByKey(_._1, (t1,t2) => t1)
          

val filter7 = group8.filter(x=>x._3 <= 7499.0)
