 ...
 */
 ...
 class Job657v0 extends GeneratableJob {
...
val join4 = group3.keyBy[Long](_._2).join(source5.keyBy[Long](_._1)).map(x => x.field1)
       

val map6 = join4.map(x=> {var count = 0; for (v1 <- x.productIterator; v2 <- x.productIterator) count+=1; x})
